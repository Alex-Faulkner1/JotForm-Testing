...
class TestResult:
    ...
    def __init__(self, test_id, test_name, form_type, scenario):
        ...
        self.screenshots = []
        self.approval_links = []
        self.last_approval_link = None  # Track last approval link for manual review
    ...
    def to_dict(self):
        ...
        return {
            ...
            'Last Approval Link': self.last_approval_link or '',  # Add for manual review
        }
...
class TestRunner:
    ...
    def select_payment_type_filter(self):
        """Ask which payment request type tests to run and filter the test suite."""
        print("\n" + "=" * 60)
        print("Payment Request Type Selection")
        print("=" * 60)
        print("1. All")
        print("2. Goods for Resale")
        print("3. Expense Payment")
        print("4. Sponsorship/Charitable Donation")
        print("5. Customer Rebate")
        print("6. Employee Expense Advance")
        print("7. Sales Ledger Refund")

        choice_map = {
            "1": "ALL",
            "2": "Goods for Resale",
            "3": "Expense Payment",
            "4": "Sponsorship/Charitable Donation",
            "5": "Customer Rebate",
            "6": "Employee Expense Advance",
            "7": "Sales Ledger Refund",
        }

        while True:
            choice = input("Select which tests to run [1-7] (default 1 = All): ").strip() or "1"
        ...
        if selected == "ALL":
            print("✅ Running tests for all payment request types")
            return

        original_count = len(self.test_cases)
        self.test_cases = [
            tc for tc in self.test_cases
            if tc.get("test_data", {}).get("payment_type") == selected
        ]
        print(f"✅ Running tests for payment request type: {selected}")
        print(f"   Filtered {original_count} → {len(self.test_cases)} test cases")
    ...
    async def fill_form(self, test_data: dict):
        ...
        # Based on working standalone script PR_Expense_Payment.py
        # ============================================================
        if test_data.get("payment_type") == "Expense Payment":
            try:
                print("\n🔹 Filling GL Codes section (Expense Payment)...")

                # Scroll to GL Codes section (multiple scrolls like working script)
                for i in range(2):
                    await self.page.evaluate("window.scrollBy(0, 300)")
                    await self.page.wait_for_timeout(200)

                # STEP 4.5.1: Click "How Many GL Codes?" RADIO BUTTON (not dropdown!)
                # The working script uses radio buttons, not a select dropdown
                try:
                    await self.page.wait_for_timeout(1000)

                    # Find the "How Many GL Codes?" section by label
                    gl_codes_clicked = False
                    labels = await self.page.query_selector_all("label")

                    for label in labels:
                        try:
                            label_text = await label.inner_text()
                            label_normalized = label_text.lower().replace('*', '').replace('?', '').strip()

                            if "how many gl code" in label_normalized:
                                # Found the label, now find the container with radio buttons
                                container = await label.evaluate_handle("el => el.closest('.form-line')")
                                if container:
                                    ...
                                    for radio in radios:
                                        ...
                                        if radio_value and radio_value.strip() == "1":
                                            # Found the "1" radio button
                                            radio_id = await radio.get_attribute('id')
                                            if radio_id:
                                                # Click the associated label
                                                label_selector = f"label[for='{radio_id}']"
                                                await self.page.click(label_selector)
                                                print(f"✓ How Many GL Codes: 1 (radio button)")
                                                gl_codes_clicked = True
                                                break
                                if gl_codes_clicked:
                                    break
                        except:
                    ...
                    if not gl_codes_clicked:
                        print(f"⚠️  Could not find 'How Many GL Codes?' radio buttons")

                except Exception as e:
                    print(f"⚠️  GL Codes radio button error: {e}")

                # STEP 4.5.2: Fill "GL Code 1" field using specific IDs from working script
                # Wait for GL Code input field to appear
                await self.page.wait_for_timeout(1500)
                print("   Waiting for GL Code input to appear...")

                try:
                    gl_code_filled = False
                    gl_code_value = test_data.get("gl_code_1", "723")

                    # Method 1: Try input_632 (primary ID from working script)
                    try:
                        gl_field = await self.page.query_selector("#input_632")
                        if gl_field and await gl_field.is_visible():
                            await gl_field.fill(gl_code_value)
                            print(f"✓ GL Code 1: {gl_code_value} (via input_632)")
                            gl_code_filled = True
                    except Exception as e:
                        pass

                    # Method 2: Try input_272 (fallback ID from working script)
                    if not gl_code_filled:
                        try:
                            gl_field = await self.page.query_selector("#input_272")
                            if gl_field and await gl_field.is_visible():
                                await gl_field.fill(gl_code_value)
                                print(f"✓ GL Code 1: {gl_code_value} (via input_272)")
                                gl_code_filled = True
                        except Exception as e:
                            pass

                    # Method 3: Try finding by label (like working script's final fallback)
                    if not gl_code_filled:
                        try:
                            labels = await self.page.query_selector_all("label")
                            for label in labels:
                                label_text = await label.inner_text()
                                if "gl code 1" in label_text.lower() or "g/l code 1" in label_text.lower():
                                    label_id = await label.get_attribute("for")
                                    if label_id:
                                        gl_input = await self.page.query_selector(f"#{label_id}")
                                        if gl_input:
                                            await gl_input.fill(gl_code_value)
                                            print(f"✓ GL Code 1: {gl_code_value} (by label)")
                                            gl_code_filled = True
                                            break
                        except:
                            pass

                    # Method 4: Index-based fallback (field order approach)
                    if not gl_code_filled:
                        try:
                            text_inputs = await self.page.query_selector_all("input[type='text']:not([readonly])")
                            visible_inputs = []
                            ...
                            # Try field index 10 (from working script)
                            if len(visible_inputs) > 10:
                                await visible_inputs[10].fill(gl_code_value)
                                print(f"✓ GL Code 1: {gl_code_value} (field index 10)")
                                gl_code_filled = True
                        except Exception as e:
                            pass

                    if not gl_code_filled:
                        print(f"⚠️  Could not fill GL Code 1 field after trying all methods")
        ...
        # This applies to both Goods for Resale AND Expense Payment
        # ============================================================
        if test_data.get("payment_type") in ["Goods for Resale", "Expense Payment"]:
            try:
                await self.page.wait_for_timeout(1000)

                if test_data.get("bank_details_on_invoice", False):
                    # Click "Yes" - bank details ARE on the invoice
                    bank_account_selector = "#label_input_607_0"
                    ...
                    if bank_radio:
                        ...
                        print("⚠️  Bank Account Details radio 'Yes' not found (#label_input_607_0)")
                else:
                    # Click "No" - need to upload bank details separately
                    bank_account_selector = "#label_input_607_1"
                    bank_radio = await self.page.query_selector(bank_account_selector)
                    if bank_radio:
                        await self.page.click(bank_account_selector)
                        print("✓ Bank Account Details on Invoice: No")
                        ...
                        try:
                            bank_details_filename = f"bank_details_{test_data.get('invoice_number', 'BD')}.pdf"
                            make_test_pdf(bank_details_filename)

                            # Field ID for bank details upload: input_191
                            file_input = await self.page.query_selector("#input_191")
                            if file_input:
                                await file_input.set_input_files(bank_details_filename)
                                print(f"✓ Uploaded Bank Account Details: {bank_details_filename}")
                            else:
                                print(f"⚠️  Bank Account Details upload field not found (#input_191)")
                        except Exception as e:
                            print(f"⚠️  Bank Account Details upload error: {e}")
                    else:
                        print("⚠️  Bank Account Details radio 'No' not found (#label_input_607_1)")
            except Exception as e:
                print(f"⚠️  Bank Account Details section error: {e}")

        # ============================================================
        # STEP 5.5: OTHER ATTACHMENTS - Expense Payment
        # "Attach Any Other Relevant Information" field (input_192)
        # ============================================================
        if test_data.get("payment_type") == "Expense Payment":
            try:
                print("\n🔹 Uploading Other Relevant Information (Expense Payment)...")
                await self.page.wait_for_timeout(1000)

                # Scroll to OTHER ATTACHMENTS section
                await self.page.evaluate("window.scrollBy(0, 300)")

                # Create other attachments PDF
                other_attachment_filename = f"other_info_{test_data.get('invoice_number', 'OTHER')}.pdf"
                make_test_pdf(other_attachment_filename)

                # Field ID for "Attach Any Other Relevant Information": input_192
                file_input = await self.page.query_selector("#input_192")
                if file_input:
                    await file_input.set_input_files(other_attachment_filename)
                    print(f"✓ Uploaded Other Relevant Information: {other_attachment_filename}")
                else:
                    print(f"⚠️  Other Relevant Information upload field not found (#input_192)")
            except Exception as e:
                print(f"⚠️  Other attachments upload error: {e}")
        ...
        # Skip for Expense Payment - already handled in Payable Documents section
        if (test_data.get("has_invoice", False) and
                test_data.get("payment_type") not in ["Goods for Resale", "Expense Payment"]):
            try:
        ...
        try:
            ...
            if payment_type == "Goods for Resale":
                # For Goods for Resale, value is calculated from payable documents
                # The "Goods (£)" field in Total Values section is typically readonly
                # Try to fill it if it's editable, but don't fail if it's not
                await self.page.evaluate("window.scrollBy(0, 400)")
                await self.page.wait_for_timeout(1000)

                # Try to find Goods field with a shorter timeout to avoid 30s wait
                try:
                    goods_field = await self.page.wait_for_selector("#input_855", timeout=3000)
                    if goods_field:
                        # Check if field is readonly/disabled
                        is_readonly = await goods_field.get_attribute("readonly")
                        is_disabled = await goods_field.get_attribute("disabled")

                        if not is_readonly and not is_disabled:
                            await self.page.fill("#input_855", test_data["value"])
                            print(f"✓ Goods Value: £{test_data['value']}")
                        else:
                            print(f"ℹ️  Goods value is readonly (auto-calculated from payable documents)")
                    else:
                        print("ℹ️  Goods value field not found (auto-calculated)")
                except:
                    # Field doesn't exist or is auto-calculated - this is normal for Goods for Resale
                    print("ℹ️  Goods for Resale value auto-calculated from payable documents")
            ...
            # Only show error for payment types that actually need a value field
            if test_data.get("payment_type") not in ["Goods for Resale", "Expense Payment"]:
    ...
    async def process_approval_stage(self, stage: dict, efs_ref: str, test_output_folder: str) -> dict:
        ...
        return {
            ...
            "action": action,
            "screenshot": screenshot_path,
            "approval_link": approval_link  # Add approval link for manual review
        }
    ...
    async def run_test_case(self, test_case: dict) -> TestResult:
        ...
        try:
            ...
            for stage in test_case["approval_workflow"]:
                ...
                # Store the last approval link for manual review
                if stage_result.get("approval_link"):
                    result.last_approval_link = stage_result["approval_link"]
                    result.approval_links.append(stage_result["approval_link"])
                ...
                # If rejected at App 2, mark that it returned to App 1
                # BUT continue processing - the next stage should be the PCM re-approval
    ...
    def generate_excel_report(self):
        ...
        headers = [
            ...
            'Expected Outcome', 'Actual Outcome', 'Error Message', 'Last Approval Link'
        ]
        ...
        for row_num, result in enumerate(self.results, 2):
            ...
            for col_num, header in enumerate(headers, 1):
                ...
                # Make Last Approval Link clickable
                if header == 'Last Approval Link' and data.get(header):
                    link = data.get(header)
                    cell.hyperlink = link
    ...
    async def run_all_tests(self):
        ...
        self.select_payment_type_filter()
        self.get_user_email()
