...
def get_approval_link_from_user(stage_name: str, efs_ref: str) -> str:
    ...
    print("  https://eel.jotform.com/edit/############?eapeo#e")
    print("=" * 60 + "\n")
...
async def submit_inputter_stage(page: Page) -> tuple[str, str]:
    ...
    # Note: We don't extract edit link here as it may not be the correct approval link
    # The approval links come via email with unique parameters
    return efs_ref, None
...
async def process_pcm_stage(page: Page, pcm_url: str, output_folder: str, user_email: str) -> str:
    ...
    if await check_already_reviewed(page):
        ...
        print("ℹ️  PCM stage already completed - moving to next stage")
        return "already_reviewed"

    # Normal flow - form needs approval
    try:
        await page.wait_for_selector("#label_input_203_0", timeout=20000)
...
async def process_rd_stage(page: Page, rd_url: str, output_folder: str) -> bool:
    ...
    if await check_already_reviewed(page):
        ...
        print("✅ RD stage already completed")
        return True

    # Normal flow - form needs approval
    try:
        await page.wait_for_selector("#label_input_249_0", timeout=20000)
...
async def run_automation():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            efs_ref, _ = await submit_inputter_stage(page)
            ...
            # Wait for workflows
            print(f"\nWaiting {WORKFLOW_WAIT_TIME / 100}s for background workflows...")
