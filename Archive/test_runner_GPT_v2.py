...
class TestRunner:
    ...
    async def fill_form(self, test_data: dict):
        ...
        # ============================================================
        if test_data.get("bank_details_on_invoice", False) and test_data.get("payment_type") in ["Goods for Resale", "Expense Payment"]:
            try:
                await self.page.wait_for_timeout(1000)
                # Click the "Yes" option for 'Are Bank Account Details on Invoice?'
...
#         # Value (different field structure for Goods for Resale)
        ...
        # ============================================================
        # STEP 7: GL Codes section for Expense Payment
        # ============================================================
        if test_data.get("payment_type") == "Expense Payment":
            try:
                # Scroll to GL Code(s) section
                await self.page.evaluate("window.scrollBy(0, 400)")
                await self.page.wait_for_timeout(1000)

                # Select "1" for "How Many GL Codes?"
                try:
                    gl_section = self.page.locator("li:has-text('How Many GL Codes?')")
                    await gl_section.wait_for(timeout=5000)
                    await gl_section.get_by_label("1").click()
                    print("✓ How Many GL Codes?: 1")
                except Exception as e:
                    # Fallback: try by label only
                    try:
                        await self.page.get_by_label("1", exact=True).click()
                        print("✓ How Many GL Codes?: 1 (fallback)")
                    except Exception:
                        print(f"ℹ️  Could not set 'How Many GL Codes?' automatically: {e}")

                # Wait for GL Code 1 field to appear and fill with 723
                try:
                    await self.page.wait_for_timeout(1000)
                    gl_code_input = self.page.get_by_label("GL Code 1")
                    await gl_code_input.wait_for(timeout=5000)
                    await gl_code_input.fill("723")
                    print("✓ GL Code 1: 723")
                except Exception as e:
                    print(f"ℹ️  Could not fill GL Code 1 automatically: {e}")
            except Exception as e:
                print(f"ℹ️  GL Codes section not available or could not be completed: {e}")
