...
class TestRunner:
    ...
    async def fill_form(self, test_data: dict):
        ...
        # ============================================================
        # GOODS FOR RESALE / EXPENSE PAYMENTS - CRITICAL ORDER!
        # ============================================================
        # STEP 1: Select "How Many Payable Documents" FIRST
        # This dropdown MUST be selected before other fields appear!
        # ============================================================

        if "payable_documents_count" in test_data:
            try:
                print("\n🔹 Filling Goods for Resale conditional fields...")
                # Wait for form to settle after payment type selection
                await self.page.wait_for_timeout(3000)

                # Select the payable documents dropdown - THIS TRIGGERS OTHER FIELDS!
                ...
                # CRITICAL: Wait for conditional fields to appear after selection
                await self.page.wait_for_timeout(3000)
                print("   Waiting for conditional fields to appear...")
            except Exception as e:
                print(f"⚠️  Payable Documents dropdown error: {e}")

        # STEP 2: Now Payment in Advance field should be visible
        ...
        if test_data.get("payment_type") == "Goods for Resale":
            try:
                ...
                # Find upload field for "Attach Your Picking Note/Sales Invoice"
                # This is in the OTHER ATTACHMENTS section
                picking_note_selectors = [
                    "input[type='file'][id*='input_558']",  # Common for other attachments
                    "input[type='file'][name*='q558']",
                    "#input_558"
                ]
                ...
                for selector in picking_note_selectors:
                    try:
                        ...
                        if file_input:
                            ...
                            if is_visible or True:  # Try even if not visible
                ...
                if not uploaded:
                    print(f"⚠️  Could not find picking note upload field")
            except Exception as e:
